...
#define _JMI_ODE_NO_STATE_H

#include <string.h>
#include "jmi_ode_solver.h"

typedef struct jmi_ode_no_state_t jmi_ode_no_state_t;

int jmi_ode_no_state_new(jmi_ode_no_state_t** integrator_ptr, jmi_ode_solver_t* solver);

jmi_ode_status_t jmi_ode_no_state_solve(jmi_ode_solver_t* solver, double time_final, int initialize);

void jmi_ode_no_state_delete(jmi_ode_solver_t* solver);

struct jmi_ode_no_state_t {
    jmi_real_t dummy;
};
