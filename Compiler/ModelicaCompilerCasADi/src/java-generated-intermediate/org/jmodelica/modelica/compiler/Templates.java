 ...
 * @declaredat C:\\stuff\\workspace\\JModelica\\Compiler\\ModelicaCompiler\\src\\jastadd\\ModelicaCompiler.jrag:2248
 */
public class Templates extends java.lang.Object {
    ...
    private String templatePath(String fileName, boolean debugSrcIsHome) {
        ...
        File home = EnvironmentUtils.getJModelicaHome();
        if (debugSrcIsHome) {
    ...
    private File templateDir() {
        return new File(EnvironmentUtils.getJModelicaHome(), "CodeGenTemplates");
    }
    ...
    public void generateXMLFiles(ModelicaCompiler mc, FClass fc, AbstractGenerator gen, File sourceDir, String name)
        ...
        mc.generateFile(fc, templatePath(getXMLTemplateName(fc), mc.debugSrcIsHome), gen, sourceDir, name + ".xml", "");
    }
        ...
        if (fmuTemplate) {
            if (C_HEADER_TEMPLATES != null) {
                for (String tpl : C_HEADER_TEMPLATES) {
                    ...
                    mc.generateFile(fc, templatePath(path, mc.debugSrcIsHome), gen, sourceDir, name + "_" + filename, "");
                }
            }
            ...
            if (C_SOURCE_TEMPLATES != null) { 
                for (String tpl : C_SOURCE_TEMPLATES) {
                    ...
                    mc.generateFile(fc, templatePath(path, mc.debugSrcIsHome), gen, sourceDir, name + "_" + filename, header);
                }
            }
        }
    }
    ...
    private void generateCFiles(ModelicaCompiler mc, FClass fc, AbstractGenerator gen, File sourceDir, String name, String header)
            throws FileNotFoundException {
        mc.generateFile(fc, templatePath(getCTemplateName(fc), mc.debugSrcIsHome), gen, sourceDir, name + ".c", header);
    }
