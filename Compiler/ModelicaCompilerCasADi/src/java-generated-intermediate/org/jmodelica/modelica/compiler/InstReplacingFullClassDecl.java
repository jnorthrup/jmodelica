...
/**
 * @ast node
 * @declaredat C:\\stuff\\workspace\\JModelica\\Compiler\\ModelicaFrontEnd\\src\\jastadd\\ast\\InstanceTree.ast:57
 * @astdecl InstReplacingFullClassDecl : InstFullClassDecl ::= <OriginalClassDecl:SrcClassDecl> OriginalInstClass:InstClassDecl <InstClassRedeclare:InstRedeclareClassNode>;
 * @production InstReplacingFullClassDecl : {@link InstFullClassDecl} ::= <span class="component">&lt;OriginalClassDecl:SrcClassDecl&gt;</span> <span class="component">OriginalInstClass:{@link InstClassDecl}</span> <span class="component">&lt;InstClassRedeclare:InstRedeclareClassNode&gt;</span>;

 */
public class InstReplacingFullClassDecl extends InstFullClassDecl implements Cloneable {
    /**
     * @aspect InstanceErrorCheck
     * @declaredat C:\\stuff\\workspace\\JModelica\\Compiler\\ModelicaFrontEnd\\src\\jastadd\\errorcheck\\ErrorCheck.jrag:1073
     */
    ...
    @ASTNodeAnnotation.Constructor(
        name = {"SrcClassDecl", "InstConstrainingClass", "InstRestriction", "InstExternal", "OriginalClassDecl", "InstClassRedeclare"},
        type = {"SrcClassDecl", "Opt<InstConstrainingClass>", "InstRestriction", "Opt<InstExternal>", "SrcClassDecl", "InstRedeclareClassNode"},
        kind = {"Token", "Opt", "Child", "Opt", "Token", "Token"}
    )
    public InstReplacingFullClassDecl(SrcClassDecl p0, Opt<InstConstrainingClass> p1, InstRestriction p2, Opt<InstExternal> p3, SrcClassDecl p4, InstRedeclareClassNode p5) {
        setSrcClassDecl(p0);
        setChild(p1, 0);
        setChild(p2, 1);
        setChild(p3, 2);
    ...
    protected boolean is$Equal(ASTNode node) {
        return super.is$Equal(node) && (tokenSrcClassDecl_SrcClassDecl == ((InstReplacingFullClassDecl) node).tokenSrcClassDecl_SrcClassDecl) && (tokenSrcClassDecl_OriginalClassDecl == ((InstReplacingFullClassDecl) node).tokenSrcClassDecl_OriginalClassDecl) && (tokenInstRedeclareClassNode_InstClassRedeclare == ((InstReplacingFullClassDecl) node).tokenInstRedeclareClassNode_InstClassRedeclare);    
    }
    /**
     ...
     * @declaredat C:\\stuff\\workspace\\JModelica\\Compiler\\ModelicaFrontEnd\\src\\jastadd\\instance\\InstanceTree.jrag:1634
     */
    @ASTNodeAnnotation.Attribute(kind=ASTNodeAnnotation.Kind.SYN, isNTA=true)
    @ASTNodeAnnotation.Source(aspect="InstanceTreeConstruction", declaredAt="C:\\stuff\\workspace\\JModelica\\Compiler\\ModelicaFrontEnd\\src\\jastadd\\instance\\InstanceTree.jrag:1634")
    ...
    public SrcAnnotatable orginalAnnotatable() {
        SrcAnnotatable orginalAnnotatable_value = getOriginalClassDecl();
    ...
    public FAccess Define_retrieveFAccess(ASTNode _callerNode, ASTNode _childNode) {
        if (_callerNode == getOriginalInstClassNoTransform()) {
            // @declaredat C:\\stuff\\workspace\\JModelica\\Compiler\\ModelicaFrontEnd\\src\\jastadd\\flattening\\FlatteningAccess.jrag:203
    ...
    public InstLookupResult<InstClassDecl> Define_lookupInstClass(ASTNode _callerNode, ASTNode _childNode, String name) {
        if (_callerNode == getAnnotationExpListNoTransform()) {
            // @declaredat C:\\stuff\\workspace\\JModelica\\Compiler\\ModelicaFrontEnd\\src\\jastadd\\util\\Annotations.jrag:269
            int childIndex = _callerNode.getIndexOfChild(_childNode);
            return getInstClassRedeclare().lookupInstClass(name);
        }
        else if (_callerNode == getInstConstrainingClassOptNoTransform()) {
            // @declaredat C:\\stuff\\workspace\\JModelica\\Compiler\\ModelicaFrontEnd\\src\\jastadd\\instance\\InstLookupClasses.jrag:94
            return lookupInstClass(name);
        }
        else if (_callerNode == getOriginalInstClassNoTransform()) {
            // @declaredat C:\\stuff\\workspace\\JModelica\\Compiler\\ModelicaFrontEnd\\src\\jastadd\\instance\\InstLookupClasses.jrag:91
            return lookupInstClass(name);
        }
        else {
